...
def trigger():
    return load_from_s3(
        path=f"{Params.bucket}/utils/triggers/{Params.general_params.get('intervals')}/model_updating_trigger.json"
    )


def run_workflow(workflow: dict) -> None:
    if trigger()['run']:
        LOGGER.info('Running MODEL UPDATING workflow:')
...
# .pytradex_venv/bin/python workflows/model_updating/model_updating_workflow.py
if __name__ == '__main__':
    # Set Up Schedule
    schedule.every().day.at('00:15:00').do(run_workflow, 'model_updating')
